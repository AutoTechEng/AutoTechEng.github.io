...
document.addEventListener("DOMContentLoaded", function() {
    ...
    var pdfViewers = document.getElementsByClassName("pdfViewer");

    // Loop through each PDF viewer container
    for (var i = 0; i < pdfViewers.length; i++) {
        var pdfViewer = pdfViewers[i];
        var pdfURL = pdfViewer.dataset.pdf;

        // Load the PDF into the current container
        loadPDF(pdfURL, pdfViewer);
    }
});